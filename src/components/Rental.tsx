...
export default function Rental() {
  ...
  const features = [
    {
      title: t(ui.rentalFeat1Title),
      desc: t(ui.rentalFeat1Desc),
    ...
    {
      title: t(ui.rentalFeat2Title),
      desc: t(ui.rentalFeat2Desc),
    ...
    {
      title: t(ui.rentalFeat3Title),
      desc: t(ui.rentalFeat3Desc),
  ...
  return (
    <section id="rental" style={{ background: studio.colors.bg, padding: isMobile ? '4rem 1.5rem' : '7rem 4rem' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? '2rem' : '5rem', alignItems: 'center' }}>
        ...
        <FadeIn direction="left">
          <div style={{ position: 'relative', borderRadius: '20px', overflow: 'hidden', aspectRatio: '4/5', background: 'linear-gradient(135deg, #F5EDE0 0%, #EAD4C5 55%, #B8826F 100%)', boxShadow: '0 30px 80px rgba(184,130,111,0.2)' }}>
            <div style={{ position: 'absolute', top: '1.5rem', left: '1.5rem', background: 'rgba(254,252,248,0.9)', backdropFilter: 'blur(8px)', padding: '0.5rem 1rem', borderRadius: '999px', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
              ...
              <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '0.6rem', fontWeight: 700, letterSpacing: '0.22em', textTransform: 'uppercase', color: studio.colors.primary }}>{t(ui.rentalLabelLv)}</p>
            </div>
        ...
        <div>
          <FadeIn delay={0.1}>
            <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '0.72rem', fontWeight: 500, letterSpacing: '0.25em', textTransform: 'uppercase', color: studio.colors.primary, marginBottom: '1rem' }}>{t(ui.rentalLabel)}</p>
            <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(2.5rem, 4vw, 4rem)', letterSpacing: '0.08em', color: studio.colors.text, lineHeight: 1, fontWeight: 400 }}>{t(ui.rentalTitle1)}</h2>
            <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(2.5rem, 4vw, 4rem)', letterSpacing: '0.08em', color: studio.colors.primary, lineHeight: 1, marginBottom: '1.5rem', fontWeight: 400 }}>{t(ui.rentalTitle2)}</h2>
