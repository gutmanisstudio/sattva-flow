...
export default function Hero() {
  ...
  return (
    <section id="home" style={{ position: 'relative', height: '100vh', minHeight: '700px', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', background: 'radial-gradient(ellipse at 30% 20%, #F5EDE0 0%, #FEFCF8 45%, #EAD4C5 100%)' }}>
      {studio.heroVideo && (
        <video ref={videoRef} autoPlay loop muted playsInline style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', zIndex: 0, opacity: 0.55 }}>
      ...
      <div style={{ position: 'relative', zIndex: 2, textAlign: 'center', padding: '0 2rem', opacity: loaded ? 1 : 0, transform: loaded ? 'translateY(0)' : 'translateY(30px)', transition: 'opacity 1.2s ease, transform 1.2s ease' }}>
        ...
        <h1 style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: isMobile ? 'clamp(3rem, 14vw, 5rem)' : 'clamp(5rem, 13vw, 10rem)', lineHeight: 0.88, color: studio.colors.text, letterSpacing: '0.04em', marginBottom: '0.2rem', fontWeight: 400 }}>{studio.name.toUpperCase()}</h1>
        <h2 style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 'clamp(1.3rem, 3vw, 2.2rem)', letterSpacing: '0.3em', color: studio.colors.primary, marginBottom: '1.5rem', fontWeight: 400 }}>{studio.tagline.toUpperCase()}</h2>
        <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '1rem', fontWeight: 300, color: studio.colors.gray, marginBottom: '0.4rem' }}>{studio.description}</p>
        ...
        <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '0.72rem', fontWeight: 500, color: studio.colors.gray, letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: '2.5rem' }}>{studio.highlight2}</p>
        ...
        <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center', flexWrap: 'wrap', flexDirection: isMobile ? 'column' as const : 'row' as const, alignItems: 'center' }}>
          <a href="#book" style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '0.78rem', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#FEFCF8', background: studio.colors.primary, padding: isMobile ? '0.75rem 1.75rem' : '0.9rem 2.2rem', borderRadius: '999px', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '0.5rem', transition: 'all 0.2s' }}
