        ...
        <FadeIn>
          <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
            <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '0.72rem', fontWeight: 500, letterSpacing: '0.3em', textTransform: 'uppercase', color: studio.colors.primary, marginBottom: '0.75rem' }}>
              {studio.scheduleMonth} — Weekly rhythm
            </p>
            <h2 style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 'clamp(3rem, 5vw, 5rem)', letterSpacing: '0.08em', color: studio.colors.text, fontWeight: 400 }}>
            ...
            <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '0.88rem', fontWeight: 300, color: studio.colors.gray, marginTop: '0.75rem', maxWidth: '520px', margin: '0.75rem auto 0', lineHeight: 1.75 }}>
              The current month at the studio. Follow <a href={studio.instagram} target="_blank" rel="noreferrer" style={{ color: studio.colors.primary, textDecoration: 'underline', textUnderlineOffset: '3px' }}>{studio.instagramHandle}</a> for last-minute changes.
            </p>
          </div>
        </FadeIn>
        ...
        <FadeIn delay={0.1}>
          <div style={{ background: studio.colors.bgAlt, borderRadius: '16px', padding: isMobile ? '1.25rem' : '2.5rem', border: '1px solid rgba(184,130,111,0.1)' }}>
            {studio.schedule.map((day, i) => (
              <div key={day.day} style={{ display: 'grid', gridTemplateColumns: isMobile ? '90px 1fr' : '180px 1fr', gap: isMobile ? '1rem' : '2.5rem', padding: '1.25rem 0', borderBottom: i !== studio.schedule.length - 1 ? '1px solid rgba(184,130,111,0.14)' : 'none', alignItems: 'start' }}>
                <p style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: isMobile ? '1.3rem' : '1.7rem', letterSpacing: '0.08em', color: studio.colors.text, fontWeight: 400 }}>{day.day}</p>
