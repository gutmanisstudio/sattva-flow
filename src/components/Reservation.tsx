    ...
    <section id="book" style={{ background: 'linear-gradient(135deg, #F5EDE0 0%, #FEFCF8 50%, #EAD4C5 100%)', padding: isMobile ? '4rem 1.5rem' : '7rem 2rem' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto', width: '100%', display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1.2fr', gap: isMobile ? '2rem' : '6rem', alignItems: 'center' }}>
        <FadeIn direction="left">
          <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '0.72rem', fontWeight: 500, letterSpacing: '0.3em', textTransform: 'uppercase', color: studio.colors.primary, marginBottom: '1rem' }}>Book</p>
          <h2 style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 'clamp(3rem, 5vw, 5rem)', letterSpacing: '0.06em', color: studio.colors.text, lineHeight: 1, marginBottom: '1.5rem', fontWeight: 400 }}>JOIN<br />A&nbsp;CLASS</h2>
        ...
        <FadeIn direction="right" delay={0.1}>
          <form onSubmit={handleSubmit} style={{ background: studio.colors.bg, padding: '3rem', borderRadius: '16px', boxShadow: '0 20px 60px rgba(184,130,111,0.12)' }}>
            <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '0.65rem', fontWeight: 600, letterSpacing: '0.25em', textTransform: 'uppercase', color: studio.colors.primary, marginBottom: '0.4rem' }}>Book a class</p>
            <h3 style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: '2.2rem', letterSpacing: '0.08em', color: studio.colors.text, marginBottom: '2rem', fontWeight: 400 }}>Reserve your spot</h3>
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
              <div><label style={lbl}>First name</label><input name="firstName" value={form.firstName} onChange={onChange} placeholder="Anna" style={inp} onFocus={e => e.target.style.borderColor = studio.colors.primary} onBlur={e => e.target.style.borderColor = 'rgba(44,36,32,0.12)'} /></div>
              <div><label style={lbl}>Last name</label><input name="lastName" value={form.lastName} onChange={onChange} placeholder="Bērziņa" style={inp} onFocus={e => e.target.style.borderColor = studio.colors.primary} onBlur={e => e.target.style.borderColor = 'rgba(44,36,32,0.12)'} /></div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
              <div><label style={lbl}>Date</label><input type="date" name="date" value={form.date} onChange={onChange} style={inp} onFocus={e => e.target.style.borderColor = studio.colors.primary} onBlur={e => e.target.style.borderColor = 'rgba(44,36,32,0.12)'} /></div>
              <div><label style={lbl}>Class</label><select name="className" value={form.className} onChange={onChange} style={inp}>{classOptions.map(c => <option key={c}>{c}</option>)}</select></div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
              <div><label style={lbl}>Spots</label><select name="spots" value={form.spots} onChange={onChange} style={inp}>{['1','2','3','4','5+'].map(g => <option key={g}>{g}</option>)}</select></div>
              <div><label style={lbl}>Phone</label><input type="tel" name="phone" value={form.phone} onChange={onChange} placeholder="+371 ..." style={inp} onFocus={e => e.target.style.borderColor = studio.colors.primary} onBlur={e => e.target.style.borderColor = 'rgba(44,36,32,0.12)'} /></div>
            </div>
            <div style={{ marginBottom: '1.5rem' }}><label style={lbl}>Notes</label>
              <textarea name="notes" value={form.notes} onChange={onChange} placeholder="New to yoga, injuries, preferred language..." rows={3} style={{ ...inp, resize: 'vertical' }} onFocus={e => e.target.style.borderColor = studio.colors.primary} onBlur={e => e.target.style.borderColor = 'rgba(44,36,32,0.12)'} />
            </div>
