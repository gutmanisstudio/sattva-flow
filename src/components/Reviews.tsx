      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        <FadeIn>
          <div style={{ textAlign: 'center', marginBottom: '4rem' }}>
            <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '0.72rem', fontWeight: 600, letterSpacing: '0.25em', textTransform: 'uppercase', color: studio.colors.primary, marginBottom: '0.75rem' }}>{t(ui.reviewsKicker)}</p>
            <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 'clamp(3rem, 6vw, 5rem)', letterSpacing: '0.1em', color: studio.colors.text }}>
              {t(ui.reviewsTitle1)} <span style={{ color: studio.colors.primary }}>{t(ui.reviewsTitle2)}</span> {t(ui.reviewsTitle3)}
            </h2>
          ...
          {studio.reviews.map((r, i) => (
            <FadeIn key={i} delay={i * 0.08}>
              <div style={{ background: studio.colors.bg, borderRadius: '16px', padding: '1.8rem', border: '1px solid rgba(184,130,111,0.08)', transition: 'transform 0.2s, box-shadow 0.2s', height: '100%' }}
                ...
                <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '0.88rem', fontWeight: 300, color: studio.colors.text, lineHeight: 1.75, marginBottom: '1.5rem' }}>&ldquo;{t(r.text)}&rdquo;</p>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.5rem' }}>
                  <div>
                    ...
                    <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '0.72rem', fontWeight: 300, color: studio.colors.gray }}>{t(r.time)}</p>
                  </div>
                  <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                    {r.tags.map((tag, j) => (
                      <span key={j} style={{ fontFamily: "'DM Sans', sans-serif", fontSize: '0.62rem', fontWeight: 600, color: studio.colors.primary, background: 'rgba(184,130,111,0.08)', padding: '0.25rem 0.7rem', borderRadius: '999px', border: '1px solid rgba(184,130,111,0.2)' }}>{t(tag)}</span>
                    ))}
                  </div>
                </div>
              </div>
